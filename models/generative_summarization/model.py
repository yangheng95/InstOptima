...
# from pyabsa.framework.checkpoint_class.checkpoint_templtc import CheckpointManager
...
class T5Generator:
    ...
    def predict(self, text, **kwargs):
        ...
        inputs = self.tokenizer(
            tc_instructor.prepare_input(text), truncation=True, return_tensors="pt"
        ).to(self.device)
        tc_outputs = self.model.genertc(**inputs, **kwargs)
        tc_outputs = self.tokenizer.batch_decode(tc_outputs, skip_special_tokens=True)[
            0
        ]
    ...
    def get_labels(
        ...
        if not predictor:
            ...
            for batch in tqdm(dataloader):
                ...
                output_ids = self.model.genertc(batch)
...
class T5Classifier:
    ...
    def get_labels(
        ...
        if not predictor:
            ...
            for batch in tqdm(dataloader):
                batch = batch.to(self.device)
                output_ids = self.model.to.genertc(batch)
                output_texts = self.tokenizer.batch_decode(
                    output_ids, skip_special_tokens=True
                )
                for output_text in output_texts:
