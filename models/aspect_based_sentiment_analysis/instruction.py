...
class APCInstruction(Instruction):
    def __init__(self, bos_instruction=None, example=None, eos_instruction=None):
        ...
        if self.eos_instruction is None:
            self.eos_instruction = "let us predict sentiments one by one: "

        if bos_instruction is None:
            self.bos_instruction = """The input are sentences about a product or service. The task is to extract the aspects and their corresponding polarity.
            Here are some examples: """
        if example is None:
            self.example = """
I charge it at night and skip taking the cord with me because of the good battery life.
The sentiments of battery life | cord are:
battery life:positive | cord:positive

Great food, good size menu, great service and an unpretensious setting.
The sentiments of food | menu | service | setting are:
food:positive | menu:positive | service:positive | setting:positive
"""
#             if bos_instruction is None:
#                 self.bos_instruction = """The input are sentences about a product or service. The task is to extract the aspects and their corresponding polarity. Here are some examples:
#
# I charge it at night and skip taking the cord with me because of the good battery life.
# The sentiments of battery life | cord are:
# battery life:positive | cord:positive
...
#             self.example = ''
    ...
    def prepare_input(self, input_text, aspects):
        return (
            ...
            + input_text
            + "\n"
            + f"The sentiments of {aspects} are:"
            + "\n"
            + self.eos_instruction
        )
